...
# Unit of length is 1 meter, analogous to index
def inverse_scanner(num_rows, num_cols, x, y, theta, meas_phi, meas_r, rmax, alpha, beta):
    """
    Generate probability values based on current measurement (one round)
    ...
    for i in range(num_rows):
        for j in range(num_cols):
            # Find range and bearing relative to the input state (x, y, theta) of sensor.
...
meas_phi = np.arange(-0.4, 0.4, 0.05) # Bearing range, not a full circle
...
for t in range(1, len(time_steps)):
    # print(f"Working at time {t}:")
    # Perform robot motion. Only add to x, y
    ...
    if (move[0] >= M - 1) or (move[1] >= N - 1) or (move[0] <= 0) or (move[1] <= 0)         or true_map[int(round(move[0])), int(round(move[1]))] == 1:
        ...
        x[0:2, t] = move
    x[2, t] = (x[2, t-1] + w[t]) % (2 * math.pi) # move theta
...
def map_update(i):
    ...
    map_ax.plot(x[1, max(0, i-10):i], x[0, max(0, i-10):i], 'bx-')  # blue x dashed

invmod_fig = plt.figure()
move_figure(invmod_fig, 500, 0)
